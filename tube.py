...
class UltraFastRenderer:
    def __init__(self, width=1280, height=720, fps=30):
        ...
        self._load_fonts()
        self.char_cache: Dict[Tuple[str, str, str], Image.Image] = {}
        self.layout_cache: Dict[int, LineLayout] = {}
        self.line_render_cache: Dict[int, np.ndarray] = {}
        
    def _load_fonts(self):
        """加载字体 - 默认拼音和汉字用相同字体，可切换为不同字体"""
        script_dir = os.path.dirname(os.path.abspath(__file__))
        
        # ========== 默认配置：拼音和汉字用相同字体 ==========
        default_font = os.path.join(script_dir, "SourceHanSansHWSC-Bold.otf")
        
        char_font_path = default_font
        pinyin_font_path = default_font
    ...
    def _calculate_layout(self, line: LyricLine) -> LineLayout:
        ...
        # 计算起始X坐标（修复：确保start_x_list长度与line_data相同）
        start_x_list = []
        ...
        for idx, data in enumerate(line_data):
            ...
            start_x = start_x_list[idx]  # 现在长度一定匹配
    ...
    def _render_char(self, char: str, pinyin: str, tone: str) -> Image.Image:
        key = (char, pinyin, tone)
        if key in self.char_cache:
            return self.char_cache[key]
        
        cell_w = self.char_font_size + 20
        cell_h = self.pinyin_font_size + self.char_font_size + 20
        img = Image.new('RGBA', (cell_w, cell_h), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)
        
        if pinyin:
            color = self.tone_colors.get(tone, (255, 255, 255))
            px = (cell_w - self._get_text_width(pinyin, self.pinyin_font)) // 2
            for dx, dy in [(-1,-1),(-1,1),(1,-1),(1,1)]:
                draw.text((px+dx, 0+dy), pinyin, font=self.pinyin_font, fill=(0,0,0))
            draw.text((px, 0), pinyin, font=self.pinyin_font, fill=color)
        
        cx = (cell_w - self._get_text_width(char, self.char_font)) // 2
        cy = self.pinyin_font_size
        
        for dx, dy in [(-1,-1),(-1,1),(1,-1),(1,1)]:
            draw.text((cx+dx, cy+dy), char, font=self.char_font, fill=(0,0,0))
        draw.text((cx, cy), char, font=self.char_font, fill=(255,255,255))
        
        self.char_cache[key] = img
        return img
    ...
    def _render_line_to_numpy(self, line: LyricLine) -> np.ndarray:
        ...
        for pos in layout.positions:
            char_img = self._render_char(pos['char'], pos['pinyin'], pos['tone'])
            paste_x = pos['char_x'] - 10
            paste_y = pos['char_y'] - self.pinyin_font_size
